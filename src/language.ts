export const language = {
  app: 'MangaLoyalty',
  errorBody: 'Oops! The server could not be reached or returned an error. Check your device and server connectivity, and then try again.',
  errorButtons: ['Disconnect', 'Retry'],
  menuBottomLibrary: 'Library',
  menuBottomRemote: 'Remote',
  menuTopDisconnect: 'Disconnect',
  menuTopRefresh: 'Refresh',
  remoteEmptyBody: 'No series have been found. When searching for a specific series, try using a different name or provider.',
  remoteEmptyTitle: '何もない',
  remoteSeriesSummary: 'No summary is available for this series.',
};

...
export class HeaderSearchBarComponent extends React.Component<{defaultSearch?: string, icon?: React.ReactElement<any>, onClose: () => void, onSearch: (value: string) => void}> {
  render() {
    return (
      <mui.Toolbar className="ios-inset-top" style={app.limiter}>
        <app.ButtonComponent title={language.iconBack} style={styles.back} onClick={() => this.props.onClose()}>
